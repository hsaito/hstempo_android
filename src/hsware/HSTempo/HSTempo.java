...
public class HSTempo extends Activity {
	...
	/**
	 * Called when the activity is first created. Initializes all necessary
	 * values here.
	 * 
	 * @author Hideki Saito
	 * @version 1.0.5
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// Currently it listens for click event, which makes this a bit
		// inaccurate.
		// Fix is underway.
		...
		// Display set
		EditText beatcountdisplay = (EditText) findViewById(R.id.BeatCountDisplay);
		beatcountdisplay.setText(String.valueOf(beatcount));

		EditText beatmonitordisplay = (EditText) findViewById(R.id.BeatMonitorDisplay);
		beatmonitordisplay.setText(String.valueOf(bpmvalue));

		// Manual Setting Button Callbacks
		findViewById(R.id.setBPMButton).setOnTouchListener(onBPMPress);
		findViewById(R.id.minusButton).setOnTouchListener(onMinusPress);
		findViewById(R.id.plusButton).setOnTouchListener(onPlusPress);
	...
	/**
	 * Perform update for change of BPM on manual
	 * 
	 * @author Hideki Saito
	 * @version 1.2.2.2
	 * @since 1.2.2.2
	...
	private OnEditorActionListener onBPMChange = new OnEditorActionListener() {

		@Override
		public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
			manualBPM = Integer.parseInt(v.getText().toString());
			return false;
		}
	};

	/**
	 * Increase Manual BPM count and continue
	 * 
	 * @author Hideki Saito
	 * @version 1.2.2.2
	 * @since 1.2.2.2
	...
	private OnTouchListener onPlusPress = new OnTouchListener() {
		public boolean onTouch(View v, MotionEvent m) {
			if (m.getAction() == MotionEvent.ACTION_DOWN) {
				EditText bpminputfield = (EditText) findViewById(R.id.bpmInputField);
				bpminputfield.setText(String.valueOf(++manualBPM));
				return true;
			}
			return false;
		}
	...
	/**
	 * Decrease Manual BPM count and continue
	 * 
	 * @author Hideki Saito
	 * @version 1.2.2.2
	 * @since 1.2.2.2
	...
	private OnTouchListener onMinusPress = new OnTouchListener() {
		public boolean onTouch(View v, MotionEvent m) {
			if (m.getAction() == MotionEvent.ACTION_DOWN) {
				if (manualBPM > 1) {
					EditText bpminputfield = (EditText) findViewById(R.id.bpmInputField);
	...
	private boolean pressEvent(View v) {
		...
		if (session_active == false) {
			...
			findViewById(R.id.bpmInputField).setFocusable(false);
			findViewById(R.id.bpmInputField).setFocusableInTouchMode(false);
			findViewById(R.id.setBPMButton).setFocusable(false);
			findViewById(R.id.setBPMButton).setFocusableInTouchMode(false);
			findViewById(R.id.bpmInputField).setFocusable(false);
			findViewById(R.id.bpmInputField).setFocusableInTouchMode(false);
			findViewById(R.id.plusButton).setFocusable(false);
			findViewById(R.id.plusButton).setFocusableInTouchMode(false);
			findViewById(R.id.minusButton).setFocusable(false);
			findViewById(R.id.minusButton).setFocusableInTouchMode(false);

			// Manual mode off
			findViewById(R.id.setBPMButton).setEnabled(false);
			findViewById(R.id.bpmInputField).setEnabled(false);
			findViewById(R.id.plusButton).setEnabled(false);
			findViewById(R.id.minusButton).setEnabled(false);
			...
			bpmvalue_double = (double) ((double) beatcount
					/ (double) ((atbeat - starttime)) * 1000 * 60);
	...
	private Runnable autoUpdateDisp = new Runnable() {
		public void run() {
			...
			// fromlast.setText((String.valueOf((int)(System.currentTimeMillis()
			// - atbeat)/1000)));
			Log.i("Calc",
					"Elapsed millsec "
	...
	void resetAll() {
		...
		// Manual mode back on
		findViewById(R.id.setBPMButton).setEnabled(true);
		findViewById(R.id.bpmInputField).setEnabled(true);
		findViewById(R.id.plusButton).setEnabled(true);
		findViewById(R.id.minusButton).setEnabled(true);
		findViewById(R.id.bpmInputField).setFocusable(true);

		findViewById(R.id.bpmInputField).setFocusable(true);
		findViewById(R.id.bpmInputField).setFocusableInTouchMode(true);
		findViewById(R.id.setBPMButton).setFocusable(true);
		findViewById(R.id.setBPMButton).setFocusableInTouchMode(true);
		findViewById(R.id.bpmInputField).setFocusable(true);
		findViewById(R.id.bpmInputField).setFocusableInTouchMode(true);
		findViewById(R.id.plusButton).setFocusable(true);
		findViewById(R.id.plusButton).setFocusableInTouchMode(true);
		findViewById(R.id.minusButton).setFocusable(true);
		findViewById(R.id.minusButton).setFocusableInTouchMode(true);
